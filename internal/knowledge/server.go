...
// reminder for the companion model about what the graph is for). The
// {first_name}/{last_name}/{pet_name} placeholders are rendered with the
// configured names in NewServer so the examples match the persona.
const serverInstructions = `A persistent knowledge graph for remembering people, animals, places, and how they relate.
Entities are typed, uniquely-named nodes (e.g. "{first_name} {last_name}" of type "person").
Observations are short free-text facts attached to an entity (e.g. "works at McDonalds").
Relations are directed, active-voice edges between entities (e.g. {first_name} —owns→ {pet_name}).
...
func NewServer(store *Store, s settings.AppSettings) *mcp.Server {
	// Example identities in the instructions and the create_relations
	// description use the configured names, so they line up with the persona
	// and the memory pass. They're rendered once here, at construction — a name
	// change needs a restart to surface. Empty names (fresh install) fall back
	// to neutral placeholders so the examples never render blank.
	ex := settings.AppSettings{
		FirstName: util.FirstNonEmpty(s.FirstName, "Ada"),
		LastName:  util.FirstNonEmpty(s.LastName, "Lovelace"),
		PetName:   util.FirstNonEmpty(s.PetName, "Mochi"),
	}
	srv := mcp.NewServer(
		&mcp.Implementation{Name: "diesel-knowledge", Title: "Diesel Knowledge Graph", Version: "1.0.0"},
		&mcp.ServerOptions{Instructions: settings.ApplyNames(ex, serverInstructions)},
	)
	...
	mcp.AddTool(srv, &mcp.Tool{
		...
		Description: settings.ApplyNames(ex, "Create directed, active-voice relations between existing entities (e.g. {first_name} owns {pet_name}). Both endpoints must already exist."),
